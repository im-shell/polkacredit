...
///         slice of the voucher's base stake ($1k / $5k / $10k), snapshots
///         the vouchee's totalPoints at open, and pays out to both sides on
///         resolve **only if** the vouchee's totalPoints grew by at least
///         `VOUCHEE_SUCCESS_THRESHOLD` during the window. On failure the
///         committed stake is slashed; no vouchee clawback is needed
///         because nothing was front-loaded.
///
///         This is a deliberate SPEC §2.3 refinement from the original
///         "front-load at open / clawback on fail" model to "credit on
///         success only." The new shape removes the auto-success exploit
///         structurally — a vouchee cannot be inflated by being vouched,
///         because the reward isn't minted until they clear the threshold.
contract VouchRegistry is Ownable2Step {
    ...
    struct VouchRecord {
        ...
        uint64 creditedToVouchee; // amount actually credited on success (filled at resolve)
        /// @notice Vouchee's totalPoints at vouch-open. The success gate
        ///         compares the current totalPoints to this snapshot at
        ///         resolution and passes iff the delta is `>=
        ///         VOUCHEE_SUCCESS_THRESHOLD`. Signed int64 because
        ///         totalPoints can go negative (inactivity penalties, etc).
        int64 voucheeTotalAtOpen;
        uint64 createdAt;
        uint64 expiresAt;
        VouchStatus status;
    }
    ...
    event VouchCreated(
        ...
        uint64 tierPoints,
        /// @notice Snapshot of vouchee's totalPoints at this block — the
        ///         basis for the delta check at resolution.
        int64 voucheeTotalAtOpen,
        uint64 createdAt,
        uint64 expiresAt
    );
    event VouchSucceeded(uint64 indexed vouchId, uint64 voucherCredited, uint64 voucheeCredited);
    /// @notice `voucheeClawback` is retained in the event signature for
    ///         indexer back-compat but is always 0 now — nothing was
    ///         front-loaded, so there's nothing to claw back.
    ...
    ///         the caller's base stake. Snapshots the vouchee's current
    ///         totalPoints; payout is deferred to `resolveVouch` and only
    ///         happens if the vouchee's totalPoints grew by at least
    ///         `VOUCHEE_SUCCESS_THRESHOLD` during the window.
    function vouch(address vouchee, uint96 committedStake) external returns (uint64) {
        ...
        // Vouchee 3-distinct ceiling. Over-cap → revert so the voucher can
        // pick a different vouchee without losing a (voucher, vouchee) slot.
        if (distinctVouchersCount[vouchee] >= MAX_DISTINCT_VOUCHERS_PER_VOUCHEE) revert VoucheeFull();

        // Snapshot the vouchee's totalPoints right now. The delta between
        // this and the post-window total is what the success gate measures.
        // No mint to the vouchee here — the reward is deferred to success.
        IPointsLedger.PointsBalance memory voucheeBalance = pointsLedger.getBalance(vouchee);
        int64 voucheeSnapshot = voucheeBalance.total;
        ...
        _vouches[vouchId] = VouchRecord({
            ...
            tierPoints: tPoints,
            creditedToVoucher: 0, // filled on resolve (success only)
            creditedToVouchee: 0, // filled on resolve (success only)
            voucheeTotalAtOpen: voucheeSnapshot,
            createdAt: createdAt,
            expiresAt: expiresAt,
        ...
        emit VouchCreated(vouchId, voucher, vouchee, committedStake, tPoints, voucheeSnapshot, createdAt, expiresAt);
        return vouchId;
    }

    /// @notice Resolve a vouch after the window closes. Permissionless —
    ///         anyone can trigger once `block.number >= expiresAt`. Success
    ///         is determined by comparing the vouchee's current totalPoints
    ///         to the snapshot taken at vouch-open. If the delta is `>=
    ///         VOUCHEE_SUCCESS_THRESHOLD` the vouch succeeds and both
    ///         parties are credited; otherwise the voucher's committed
    ///         stake is slashed to treasury.
    /// @dev    Post-SPEC-refinement: the success decision is pure on-chain
    ///         math over authoritative ledger state, no oracle signature
    ///         required. Disputers who disagree with the outcome use the
    ///         standard dispute path (e.g., argue totalPoints itself is
    ///         wrong via `WrongTotalPointsSum`).
    ...
    function resolveVouch(uint64 vouchId) external {
        ...
        // fail the threshold check.
        int64 delta = voucheeBalance.total - v.voucheeTotalAtOpen;
        bool succeeded = delta >= int64(uint64(VOUCHEE_SUCCESS_THRESHOLD));
        ...
        if (succeeded) {
            ...
            uint64 voucherCredit = v.tierPoints > remaining ? remaining : v.tierPoints;

            v.creditedToVoucher = voucherCredit;
            if (voucherCredit > 0) {
                voucherLifetimeCredited[v.voucher] = already + voucherCredit;
                pointsLedger.mintPoints(v.voucher, voucherCredit, "vouch_given");
            }

            // Vouchee receives the full tier payout on success. The old
            // "front-load at open" model is gone — this is the single
            // credit event for the vouchee.
            v.creditedToVouchee = v.tierPoints;
            pointsLedger.mintPoints(v.vouchee, v.tierPoints, "vouch_received");

            // Return committed stake to voucher.
            stakingVault.uncommitStake(v.voucher, v.committedStake);

            emit VouchSucceeded(vouchId, voucherCredit, v.tierPoints);
        } else {
            v.status = VouchStatus.Failed;

            // SPEC §3.1 voucher: slash committed stake. No vouchee clawback
            // because nothing was front-loaded — `creditedToVouchee` is 0.
            stakingVault.slashStake(v.voucher, v.committedStake);

            emit VouchFailed(vouchId, 0, 0, v.committedStake);
        }
    ...
    ///         the vouch was active. Applies SPEC §3.1 voucher-side
    ///         treatment: slash the committed stake to treasury. No vouchee
    ///         clawback is needed because the vouch_received reward is now
    ///         deferred to successful resolve — an Active vouch has never
    ///         minted anything to the vouchee.
    ///
    ///         The separate −100 flat loan-default penalty (SPEC §3.2) is
    ///         emitted by the loan protocol's own event stream and applied
    ///         by the indexer, not here.
    ///
    ...
    function reportDefault(uint64 vouchId) external {
        ...
        // Slash committed stake to treasury. Under deferred-credit, there's
        // nothing on the vouchee side to reverse.
        stakingVault.slashStake(v.voucher, v.committedStake);
